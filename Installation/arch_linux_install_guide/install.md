...
本节提供一个在配备了Nvidia独立显卡的Intel平台笔记本（常见游戏本均为该组合）上安装桌面环境的例子。

本节提供了Xfce4和Deepin Desktop Environment(DDE)的安装示例。DDE的动画效果、外观设计等方面体验更佳，而Xfce4简洁而又稳定，读者可以根据自己的偏好自由选择。

注意，本节所有命令需要root权限，建议使用root用户登录。

### 概述

我们假定的平台----配备了Nvidia独立显卡的Intel平台笔记本有两个显卡，分别为Nvidia独立显卡和Intel核显
...
如果读者选择Xfce4，找到 `[Seat:*]`，反注释其中的`greeter-session`配置项，并将其值改为 `lightdm-gtk-greeter`。

如果读者选择DDE，值应为 `lightdm-deepin-greeter`。
...
这将设置lightdm登录会话为Deepin自带的`lightdm-deepin-greeter`。
...
启用NetworkManager。为防止与dhcpcd冲突，确保禁用dhcpcd。
...
如果读者选择了Xfce4，还需要安装nm-applet系统托盘，方便图形化管理。而Deepin已经自带管理界面。

```shell
# pacman -S network-manager-applet
```

如果读者选择了DDE，那么需要禁用NetworkManager默认开启的MAC地址随机化，否则连接Wifi时可能出现故障，无法连接。
...
关于Deepin的更多问题，参见[Deepin Troubleshooting - ArchWiki](https://wiki.archlinux.org/index.php/Deepin_Desktop_Environment#Troubleshooting)
...
>```
>
>警告：建议所有用户都经常性的更新系统。否则系统可能陷入不可用状态。
>
